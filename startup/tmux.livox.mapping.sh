...
run_pane_cmd 3 "cd $PROJECT_DIR && source ./setup.sh && ros2 launch go2_nav go2_rtabmap.livox.mapping.launch.py"

# Separate window (tab): Livox MID-360 driver
if tmux list-windows -t $SESSION -F '#{window_name}' | grep -q '^livox$'; then
    tmux kill-window -t $SESSION:livox
fi
tmux new-window -t $SESSION -n livox -c "$PROJECT_DIR"
tmux send-keys -t "$SESSION:livox" "cd $PROJECT_DIR && source ./setup.sh && ros2 launch go2_nav livox_mid360.launch.py" C-m
